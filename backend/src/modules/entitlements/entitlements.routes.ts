...
import { getMyEbooks, getMyEbookIds, requestDownload, serveFile } from './entitlements.controller';

// Router cho /me/ebooks — yêu cầu auth + role 'user'
export const entitlementsRouter = Router();

entitlementsRouter.get('/', auth, requireRole('user'), asyncHandler(getMyEbooks));
entitlementsRouter.get('/ids', auth, requireRole('user'), asyncHandler(getMyEbookIds));
